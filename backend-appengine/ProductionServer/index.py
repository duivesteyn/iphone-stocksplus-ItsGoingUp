...
class NewNotificationEngine(webapp.RequestHandler):
  def get(self):
    ...
    if notificationCount <> 0: 
     ...
     logging.debug('listsOfStocksToParse' + str(listsOfStocksToParse))
     #actualListOfStocksToParse = [] (Changed in 1.11)
     for i in range(0, len(listsOfStocksToParse)): 	
       actualListOfStocksToParse = []													 
       listOfStocksToParse= listsOfStocksToParse[i]
       stockurlstring=''
       for k in range(0,len(listOfStocksToParse)):      #k is the STOCK TICKER
	     if listOfStocksToParse[k] is not None:
	      data = memcache.get(str(listOfStocksToParse[k])) 
	      logging.debug('checking for memcache for stock ' + str(listOfStocksToParse[k]) + ' exists - ' + str(data))
       ...
       for k in range(0,len(actualListOfStocksToParse)):												#Run through list again and put them all into memcache
        ...
        if stock <> None:                                                                               #Dont process type None
	     logging.debug('k:' + str(k) + ' stockValue ' + str(resultText[k]))
	     ...
	     logging.debug('Hybrid Notification Engine - Stock Value Fetch - '+stock +' '+stockValue +"  " + str(k+1) + "/" + str(len(listOfStocksToParse)))   #debug code
     ...
     memcache.set_multi({ "checks": str(notificationCount), "checkTime": str(engineTime), "sentLastRun": str(notificationsSent), "speed" : str(engineRate), "runDate": str(engineStopTime) }, key_prefix="notificationenginestat_", time=8640000)
...
class StatsHandler(webapp.RequestHandler):
  def get(self):
    ...
    #Engine Stats
    engineStatRunDate = memcache.get('notificationenginestat_runDate')
    engineStatChecks = memcache.get('notificationenginestat_checks')
    engineStatCheckTime = memcache.get('notificationenginestat_checkTime')
    engineStatSentLastRun = memcache.get('notificationenginestat_sentLastRun')
    engineStatSpeed = memcache.get('notificationenginestat_speed')
       

    doRender(self, 'stats.htm', {'environment': environment,'stockCount':stockCount,'notification_count': notificationCount, 'notification_totalcount':notificationTotalSentCount, 'device_count': deviceCount, 'engine_rundate' : engineStatRunDate, 'engine_checks':engineStatChecks, 'engine_checktime':engineStatCheckTime, 'engine_lastrun':engineStatSentLastRun, 'engine_speed':engineStatSpeed })
	
class NotifyDevPhoneWithProductionKeys(webapp.RequestHandler):
  def get(self):
    self.session = Session()
    que = db.Query(Notification)
    notification_list = que.fetch(limit=500)
    #functions.sendApplePushNotificationTest('test') #old boring code
    functions.sendApplePushNotificationTestProductionKeys('test') 
    doRender(self,'notifications.htm',{'msg' : 'Notification Sent - Check Development iPhone - Used Production Push Keys','notification_list': notification_list})
    
class NotifyDevPhoneWithDevelopmentKeys(webapp.RequestHandler):
  def get(self):
    self.session = Session()
    que = db.Query(Notification)
    notification_list = que.fetch(limit=500)
    functions.sendApplePushNotificationTestDevelopmentKeys('test')  
    doRender(self,'notifications.htm',{'msg' : 'Notification Sent - Check Development iPhone - Used Development Push Keys','notification_list': notification_list})
...
def main():
  application = webapp.WSGIApplication([
     ...
     ('/notify/devPhone/ProductionKeys',NotifyDevPhoneWithProductionKeys),
     ('/notify/devPhone/DevelopmentKeys',NotifyDevPhoneWithDevelopmentKeys),
