...
- (id)initWithPageNumber:(int)page
{
    ...
    startDateforCodeExecutionTiming = [[NSDate date] retain];
    NSLog(@"============");
...
- (void)viewDidLoad
{
    ...
    tableView.separatorStyle = UITableViewCellSeparatorStyleNone;   //Remove Seperator Lines

    //UI - Setup AboutView Positioning (3.5" and 4.0" screen)
    ...
    //UI - Set About View Version Text
    ...
    aboutAppNameLabel.font = [UIFont fontWithName:@"Avenir-Heavy" size:17.0];
    ...
    //Set a repeating timer for stock updates
    ...
    //One time run messages - App Welcome. Setting Secret Bits (etc) (Done in seperate thread)
    NSInvocationOperation *operationFirstRunMethod = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(firstRunMessages) object:nil];
    [operationQueue addOperation:operationFirstRunMethod];     // Add the operation to the queue (replaces [self firstRunMessages])
    [operationFirstRunMethod release];
    
    //One time data consistency check in NotificationIDs
    NSInvocationOperation *operationFillAnyEmptyNotificationIDs = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(fillAnyEmptyNotificationIDs) object:nil];
    [operationQueue addOperation:operationFillAnyEmptyNotificationIDs];     // Add the operation to the queue (replaces [self operationFillAnyEmptyNotificationIDs])
    [operationFillAnyEmptyNotificationIDs release];
...
// Method Generates a random string of Hex (for notification IDs)
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    CGRect cgRect =[[UIScreen mainScreen] bounds];
    CGSize cgSize = cgRect.size;
    tableView.frame = CGRectMake(0, 44, 320,cgSize.height-44-25-20+1);
    
    NSLog(@"Finished SummaryViewController viewWillAppear. ");
}

-(void)fillAnyEmptyNotificationIDs{
    ...
    NSLog(@"Checking Consistency in ContentArray NotificationIDs");
    
    if ([contentArray count] > 0) {
        
        for (int t = 0; t < [contentArray count]; t++) {
            NSString *notificationID = [[contentArray objectAtIndex:t] objectForKey:@"Notification ID"];
            NSLog(@"notificationID - %@ : %@",[[contentArray objectAtIndex:t] objectForKey:@"Symbol"],notificationID);
            
            if ([notificationID length] == 0)    {
                NSString *hexStr = [self genRandStringLength:12];
                [[contentArray objectAtIndex:t] setObject:hexStr forKey:@"Notification ID"];
                NSString *notificationIDUpdated = [[contentArray objectAtIndex:t] objectForKey:@"Notification ID"];
                NSLog(@"notificationID - Assigning %@ in contentArray to  %@",notificationIDUpdated,[[contentArray objectAtIndex:t] objectForKey:@"Symbol"]);
                notificationID = [[contentArray objectAtIndex:t] objectForKey:@"Notification ID"];
            }
        }
...
-(void)viewDidAppear:(BOOL)animated {
    ...
    NSTimeInterval executionTime = [methodFinish timeIntervalSinceDate:startDateforCodeExecutionTiming];
    NSLog(@"Execution Time - Init to end of viewDidAppear: %f", executionTime);
...
-(void)firstRunMessages{
    //Method historically cehecked for Jalibreak (dont care now)
    //Method manages any special changes from version to version
    //Method sends the Welcome Screen the first launch
    //CHanged in V1.2 to run as a seperate thread
    ...
    if(Ver == nil || [Ver compare:CurVer] != 0)
    {
        ...
        //Add Object to recognise early user. Early users are already paid users, deserve the app for free (paid user before free switch) (Added V1.1)
        [Def setObject:@"1" forKey:@"EarlyUser"];
...
-(void)timerCallUpdateStocks{
    ...
    //[self updateStockData];
    NSInvocationOperation *operationUpdateStockData = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(updateStockData) object:nil];
    [operationQueue addOperation:operationUpdateStockData];     // Add the operation to the queue (replaces [self operationFillAnyEmptyNotificationIDs])
    [operationUpdateStockData release];
}
...
-(void)updateStockData {
    ...
    if (noInternetConnection == 0) {
        ...
        //Check there hasnt been a recent update (30sec)
        now = [[NSDate date] retain];    //current time
        int secondsSinceLastUpdate = [now timeIntervalSinceDate:lastUpdate];
        NSLog(@"secondsSinceLastUpdate: %d",secondsSinceLastUpdate);
        
        if (secondsSinceLastUpdate > 15  || secondsSinceLastUpdate < -2000000000 ) {
            ...
            //Send local banner message
            if (bannerNotShowing){
                [[MTStatusBarOverlay sharedInstance] postMessage:@"Updating Stocks..."];
                [self performSelector:@selector(changeTextAnimatedFinish) withObject:nil afterDelay:1.5];
            }
        }
    
        lastUpdate = [[NSDate date] retain];;
        NSLog(@"lastUpdate updateStockData: %@",lastUpdate);
    } else  {
        //no internet!!!
        NSLog(@"no internet. cant update now.") ;
...
forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"in aTableView commitEditingStyle editingStyle forRowAtIndexPath");
	//Deleting Row
...
- (UITableViewCell *)tableView:(UITableView *)tableview cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil) {
        ...
        //Label - Stock Value
		UILabel *labelStockPrice = [[UILabel alloc] initWithFrame:CGRectMake(210, 11, 105, 40)];
        labelStockPrice.backgroundColor = [UIColor clearColor];
        labelStockPrice.font = [UIFont fontWithName:@"Avenir-Heavy" size:20];
        ...
        labelStockPriceChange.font = [UIFont fontWithName:@"Avenir" size:14];
    ...
    //Font
    cell.textLabel.font = [UIFont fontWithName:@"Avenir-Heavy" size:19];
...
- (IBAction)facebookCallToAction{
    ...
    //Flurry
    [Flurry logEvent:@"Sent to Facebook"];
    
    //Open FB
    NSURL *fanPageURL = [NSURL URLWithString:@"fb://profile/403619086347776"];  //correct URL for stocksPlus
    if (![[UIApplication sharedApplication] openURL: fanPageURL]) {
        //fanPageURL failed to open.  Open the website in Safari instead
        NSURL *webURL = [NSURL URLWithString:@"http://www.facebook.com/StocksPlus"];
...
-(void)showNoInternetBar:(NSNotification *) notification {   
    ...
    noInternetBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 24)];
