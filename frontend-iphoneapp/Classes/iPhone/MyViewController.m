...
- (void)viewDidLoad
{
    ...
    //If you want to have the Stock Details Section bigger, and the chart smaller uncomment the following:
...
-(void)viewWillAppear:(BOOL)animated{
    //NSLog(@"Page Number %d", pageNumber);
    //Keep this method light to enhance aparent loading time. (some stuff moved to viewDidAppear)
...
-(void)viewDidAppear:(BOOL)animated {
    ...
    if (verbose) NSLog(@"Read Notification Status: %@",var1);
    ...
    stockTargetPriceView.userInteractionEnabled = NO; //Disable Stock Target View Initially. This is later turned on/off
...
-(void)getForCachedChartData {
    NSLog(@"getForCachedChartData - Looking for cache for %@",[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]);
	...
	if (fileExists == 1)  {
        NSLog(@"getForCachedChartData - 365D Graph Data exists for %@",[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]);
...
-(void)graphSlicer:(int)days{
    ...
    if (verbose) NSLog(@"Number of Data for Stock: %d",stockDataCount);
    ...
    if (days == 365) {
        ...
        if ([graphData count] > 124) {
            ...
            if (verbose) NSLog(@"Not enough data;");
        ...
        if (verbose) NSLog(@"theRange.location : %d  . theRange.length: %d",theRange.location,theRange.length);
        ...
        partialGraphData = [graphData subarrayWithRange:theRange];
        partialScaleData = [scaleData subarrayWithRange:theRange];
        if (verbose) NSLog(@"2 theRange.location : %d  . theRange.length: %d",theRange.location,theRange.length);   
...
-(void)updateStockData{
    NSLog(@"======================="); NSLog(@"in updateStockData pageNumber: %d of %d",pageNumber,[contentArray count]); NSLog(@"=======================");
    
    if (pageNumber > [contentArray count]) {
        //Yes, This is a work around. TODO: Find the actual source of having pageNUmber>content Array count when deleting an object. (17.02.2013)
        NSLog(@"Requesting Load of Stock: ERROR. This is out of range. Dropping Request");
    } else {
...
- (void)stockDetailsDidLoad:(YFStockDetailsLoader *)aDetailsLoader
{
   ...
   // NSLog(@"%@", [contentArray objectAtIndex:pageNumber-1]);
    ...
    //Graph Data Lookup Method (v1.1) (Combined. 1D + 5D)
...
-(void)updateViewDataFields{
    ...
    //Updates all labels in view //This bit written on 24/12/2011 in Australia!
    if ([liveDataArray objectForKey:[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]]) {
       ...
       // NSString *stockMktCapStrVar = convertValueToStringWithMagnitude(stockMktCapfloat);
        ...
        if (verbose) NSLog(@"changeValue = units %f * (stockValueVar %@ - stockBuyVar %@)",units,stockValueVar,buy);
...
-(void)updateViewTargetBar{
    ...
    //Display  Above/Below Setting (Note needs to gracefully handle no entry. This code added in 1.1 as bug correction)
    ...
    //Get Correct Repeat Setting (This code added in 1.1 as bug correction)
    ...
    if ([liveDataArray objectForKey:[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]]) {
        ...
        if (notificationAboveBelow.selectedSegmentIndex == 0) {
            ...
            if (targetbarCurrentPrice >= targetPrice) {
                if (verbose) NSLog(@"TargetBar: Current Stock is Above Target Price");
                ...
                if (verbose) NSLog(@"TargetBar: Current Stock is Between Buy Price and Sell Target");
                ...
                if (verbose) NSLog(@"TargetBar: Current Stock is Below Buy Price");
            ...
            if (targetbarCurrentPrice > targetPrice) {
                if (verbose) NSLog(@"TargetBar: Current Stock Price is greater than Target Price");
                targetBarImage = [UIImage imageNamed: @"wrapping-correction-red.png"];                             //Bar is Red
                targetBarImageEditButton = [UIImage imageNamed: @"button-edit-red.png"];                           //Bar is Red
                stockTargetPrice.text =  (targetPrice >=100) ? [NSMutableString stringWithFormat:@"Stock Above Target: %.0f", targetPrice] : [NSMutableString stringWithFormat:@"Stock Above Target: %.2f", targetPrice];
                
            } else if (targetbarCurrentPrice <= targetPrice) {
                if (verbose) NSLog(@"TargetBar: Current Stock is Below Target Price");
                targetBarImage = [UIImage imageNamed: @"wrapping-correction-green.png"];                                //Bar is Green
                targetBarImageEditButton = [UIImage imageNamed: @"button-edit-green.png"];                              //Bar is Green
        ...
        if (verbose) NSLog(@"TargetBar: No information on Live Data Available - Using Grey Bar");
    }
...
-(IBAction)notificationDataUpdate:(id)sender {
    ...
    //Repeat Functionality Data Switch (added in V1.1)
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    for (int i = 0; i < [string length]; i++) {
        ...
        if ([myCharSet characterIsMember:c]) {
            ...
            if (verbose) NSLog(@"sepPeriod: %@ sepComma: %@" ,sepPeriod ,sepComma );
            if (verbose) NSLog(@"sepPeriod: %d sepComma: %d" ,[sepPeriod count],[sepComma count]);
            int totalDecimals = [sepComma count] + [sepPeriod count];
             if (verbose)NSLog(@"totalDecimals: %d",totalDecimals);
...
- (void)textFieldDidChange:(UITextField *)textField{
     if (verbose) NSLog(@"in textFieldDidChange");
    //[self notificationDataUpdate:nil];  //update notification text
}
...
-(void)noInternetMode:(NSNotification *) notification {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"showNoInternetBar" object:nil];
             ...
             [stockStatsView addSubview:noInternetView];
    
    
                notificationSwitch.enabled = NO; //disabled ability to toggle notification
    
             noInternetView.frame = CGRectMake(0, 1, noInternetView.frame.size.width, noInternetView.frame.size.height);
...
-(void)hideNoInternetMode:(NSNotification *) notification {
    ...
    if (verbose) NSLog(@"View:%@ - in hideNoInternetMode",[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]);
    ...
    notificationSwitch.enabled = YES; //re allow ability to toggle notification
...
-(void)highResolutionDataLookup{
    ...
    NSDate *now = [[NSDate date] retain];    //current time
    NSLog(@"TimingOptimisation - Finish time: %@",now);
}
...
-(void)highResolutionDataLookupOneDay{
    ...
    NSLog(@"Finished Executing highResolutionDataLookupOneDay - %d - High Res Data now available for: %@",pageNumber-1,[[contentArray objectAtIndex:pageNumber-1] objectForKey:@"Symbol"]);
...
-(IBAction)chartButton1D:(id)sender{
    ...
    if (verbose) NSLog(@"Stock plannedOpenTimeAdjusted: %@",plannedOpenTimeAdjusted);
    //Planned Close Time
    ...
    if (verbose) NSLog(@"Stock plannedCloseTimeAdjusted: %@",plannedCloseTimeAdjusted);    
    //Latest Time
    ...
    if (verbose) NSLog(@"Stock currentDate and currentDateAdjusted: %@,%@",currentDate,currentDateAdjusted);    
    ...
    if (verbose)NSLog(@"Mid Date: %@",midDateStockOpenCloseAdjusted);
    ...
    if (verbose) NSLog(@"Time Diffs: %f, %f. Day Percent %.0f",secondsBetweenOpenAndClose,secondsBetweenOpenAndNow,percentageOfDayThatStockDataIsAvailable);
