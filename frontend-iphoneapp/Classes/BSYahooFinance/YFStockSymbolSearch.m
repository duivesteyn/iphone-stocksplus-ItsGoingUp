...
- (void)requestDidFinish:(ASIHTTPRequest *)request
{
	...
	if (!parsedDictionary) {
        ...
        NSLog(@"Returned Data - parsedDictionary: %@",[parsedDictionary objectForKey:@"ResultSet"]);
        ...
        //I dont want indexes to show up in the stocks list. I have removed f eks. ^DJI
        //NSLog(@"Checking Symbol: %@",symbol.symbol);
        if ([symbol.symbol rangeOfString:@"^"].location == NSNotFound) {
            [result addObject:symbol];  //add to list
            [symbol release];
        } else NSLog(@"string %@ contains ^",symbol.symbol);
